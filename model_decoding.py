...
class EEG2TextModel(nn.Module):
    def __init__(self, pretrained_model, input_dim=840, embedding_dim=1024, nhead=8, ff_dim=2048):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(
            d_model=input_dim, nhead=nhead, dim_feedforward=ff_dim, batch_first=True
        )
        self.encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=6)

        # Fully connected layer to match embedding size
        self.embedding_fc = nn.Sequential(
            nn.Linear(input_dim, embedding_dim),
            nn.ReLU()
        )

    def forward_encoder(self, eeg_features, attention_mask):
        """Encode EEG features to embedding space."""
        encoded_features = self.encoder(eeg_features, src_key_padding_mask=attention_mask)
        embeddings = self.embedding_fc(encoded_features)
        return embeddings

    @torch.no_grad()
    def generate_text(
        self,
        eeg_features, attention_mask, labels,
        generation_config=None, logits_processor=None, stopping_criteria=None,
        prefix_allowed_tokens_fn=None, synced_gpus=None,
        assistant_model=None, streamer=None,
        **kwargs
    ):
        """Generate text from EEG embeddings."""
        embeddings = self.forward_encoder(eeg_features, attention_mask)
...
class T5Translator(nn.Module):
    def __init__(self, pretrained_layers, in_feature=840, decoder_embedding_size=2048, additional_encoder_nhead=16,
    ...
    def generate(
        ...
        input_ids = self.tokenizer("transcribe in English: ", return_tensors="pt").input_ids.to(
            encoded_embedding.device)
        ...
        input_masks_batch = torch.cat((torch.ones(encoded_embedding.size(0), task_embedding.size(1)).to(
            encoded_embedding.device), input_masks_batch), dim=1)
